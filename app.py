...
#Data_Url = r"C:\Users\rzn\Desktop\NYC Vehicle Collision Analytics\NYC_Vehicle_Collision_small.csv"
...
def read_csv_from_github(url):
    """Reads a CSV file from GitHub and returns a Pandas DataFrame."""

    response = requests.get(url)
    df = pd.read_csv(io.StringIO(response.content.decode()))
    return df

csv_file_url = "https://raw.githubusercontent.com/{}/{}/main/NYC_Vehicle_Collision_small.csv".format(
    "rzn-git", "Streamlit_Practice"
)

#df = read_csv_from_github(csv_file_url)



df = pd.read_csv(csv_file_url, nrows = 100000, parse_dates=[['CRASH_DATE','CRASH_TIME']])
